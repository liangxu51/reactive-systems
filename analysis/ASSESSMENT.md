...
- ~~`inventory-service`'s Mongo seed loader (`AsyncApplication.java:23-31`) is self-documented as broken (`// TODO: This does not work for reactive repositories from Mongo`) — `data.json` never loads under any deployment, confirmed by the Helm README's separate manual `mongo --eval` seeding workaround.~~ **✅ Fixed in PR #14** — see Technical Debt #6.
...
- ~~A documented-but-unreproduced runtime bug: `k8s/helm/reactive-systems/README.md:82-89` reports `shipping-service` failing to publish `SHIPPING_SUCCESS`/`SHIPPING_FAILURE` with a `LocalDate` Jackson `InvalidDefinitionException`, consistent with the confirmed absence of `jackson-datatype-jsr310` in any `pom.xml`. Flagged as **Confidence: Medium** — needs SME reproduction.~~ **Reproduced and fixed in PR #12** (issue #11). Reproduction confirmed the missing-dependency diagnosis was incomplete: the actual defect was a `LocalDate` vs `Date` type mismatch between `shipping-service`'s and `order-service`'s duplicated `Order.shippingDate` fields, and it did far more damage than a failed publish — it permanently poisoned `order-service`'s Kafka consumer once the type mismatch actually got serialized (see Technical Debt #12).
...
1. ~~**`inventory-service` and `shipping-service` have zero automated tests**, yet own the two riskiest business rules in the saga: stock check-and-decrement (`ProductService.java:26-45`) and the hardcoded 10:00–18:00 shipping-acceptance window (`ShippingService.java:29-30`).~~ **✅ Fixed in PR #1** — `ProductServiceUnitTest` and `ShippingServiceUnitTest` added.
2. ~~**`.subscribe()` with no `onError` consumer** in the inventory and shipping Kafka consumers (`inventory-service/.../async/consumer/OrderConsumer.java:30-54`, `shipping-service/.../async/consumer/OrderConsumer.java:30-42`) — `doOnError` only peeks, it doesn't handle; an unhandled terminal error can destabilize the listener and prevent the very failure-notification the saga depends on.~~ **✅ Fixed in PR #5**.
3. ~~**No consumer anywhere handles `INVENTORY_FAILURE` or `INVENTORY_REVERT_FAILURE`** — `order-service`'s `NEXT_STATUS` map (`async/consumer/OrderConsumer.java:20-23`) only covers the happy-path + `SHIPPING_FAILURE`. A failed stock revert is permanently silent with no retry or alert.~~ **✅ Fixed in PR #6** — both statuses are now logged at ERROR as terminal/unrecoverable (no compensating action exists, so this is visibility rather than a new retry path).
4. ~~**Frontend API base URLs are hardcoded to `localhost`** (`frontend/src/api/ordersApi.js:1-2`, `useOrderStream.js:3`), which silently breaks the Helm/k8s deployment this same repo ships — the Helm README works around it by instructing `kubectl port-forward svc/order-service 8080:8080` rather than fixing the frontend.~~ **✅ Fixed in PR #8**.
5. ~~**Non-atomic read-check-decrement on stock** in `ProductService.handleOrder` (`inventory-service/.../ProductService.java:26-45`) relies on `@Transactional` + `ReactiveMongoTransactionManager` to catch write conflicts at commit time, but nothing retries on conflict — concurrent orders for the same product surface as generic consumer errors instead of controlled oversell prevention.~~ **✅ Fixed in PR #7** — retries on transient Mongo write conflicts.
6. ~~**Dead seed-data loader** — see Dangling References above; self-documented broken code that looks functional to a new contributor.~~ **✅ Fixed in PR #14** — removed the non-functional `Jackson2RepositoryPopulatorFactoryBean` bean; `data.json` is retained solely as the source for the manual `mongo --eval` seed workflow already documented in the Helm README.
7. **No Kafka error-handling/dead-letter strategy** anywhere (`ErrorHandler|RetryTemplate|DeadLetter` — zero matches repo-wide). A poison-pill message on the single shared `orders` topic has no DLT route. *(Still open — see the newly-found item #12 below, which is a concrete instance of exactly this gap: a single bad message permanently wedged `order-service`'s consumer.)*
8. **Wildcard CORS with no authentication** on every controller (`@CrossOrigin(origins = "*")`, no Spring Security dependency anywhere) — also see Security Findings below. *(Still open.)*
9. **Helm chart pins every image to mutable `:latest`** with `imagePullPolicy: IfNotPresent` (`values.yaml:9,26-63`) — a rebuilt image with the same tag won't be re-pulled, producing non-reproducible deployments. *(Still open.)*
10. ~~**`order-service-vt` can publish `INITIATION_SUCCESS` and then locally downgrade the same order to `FAILURE`** on a later save exception (`vt/service/OrderService.java:26-41`) — the already-published success event has no retraction, a smaller-scope version of the dual-write problem the saga otherwise handles correctly.~~ **✅ Fixed in PR #15** — the `INITIATION_SUCCESS` status is now durably saved before the Kafka publish happens; a publish failure after a successful persist is logged rather than allowed to downgrade the already-committed state.

**Found and fixed after this assessment (not in the original ranked list — both were saga-completion blockers, more severe than anything above):**

11. ~~**`order-service` forwarded a stripped `Order` (id + status only) at every saga hop**, dropping `lineItems`/`shippingAddress`/`userId`/`total` — `inventory-service` NPE'd on the null `lineItems` and `shipping-service` silently persisted shipments with a `null` address. No order placed through the UI or API could ever complete the saga.~~ **✅ Fixed in PR #10** (filed as issue #9).
12. ~~**Confirms and supersedes Dangling Reference #3 below**: `shipping-service`'s duplicated `Order.shippingDate` was typed `java.time.LocalDate` while `order-service`'s copy is `java.util.Date`. Once real data started flowing (after #11 above was fixed), the type mismatch meant `shipping-service` couldn't even serialize the field, and once that was naively patched by adding `jackson-datatype-jsr310`, the default array-encoded `LocalDate` JSON couldn't be deserialized by `order-service`'s `Date` field either — throwing a `SerializationException` that Spring Kafka's default error handler can't recover from, **permanently poisoning `order-service`'s entire Kafka consumer** (every subsequent order got stuck, not just the one that hit it — a concrete manifestation of debt item #7's missing DLT strategy).~~ **✅ Fixed in PR #12** (filed as issue #11) — aligned the wire-facing type to `Date` across both services rather than adding a Jackson module.

Not counted as debt (intentional, documented design choices): the `Order`/`LineItem`/`Address`/`OrderStatus`/`ObjectIdSerializer` duplication across services (explicit in CLAUDE.md, for independent deployability), and the dual Jackson 2/3 serializer annotations (a managed consequence of Spring Boot 4's Jackson 3 default alongside Spring Kafka's continued Jackson 2 dependency). **Caveat surfaced by item #12**: duplication is fine for structure, but the *wire-format types* of duplicated fields must still match byte-for-byte across services — a divergence like `LocalDate` vs `Date` on the same logical field compiles cleanly in each module alone and only breaks at runtime, across the Kafka boundary.
