...
// its side effect (e.g. decrementing stock twice). The id IS the dedup key -
// MongoDB's built-in unique _id index enforces "exactly one processed marker
// per (order, status)" with no extra index to declare or maintain.
...
@Data
@Document("inventory_processed_event")
